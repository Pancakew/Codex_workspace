...
parser = argparse.ArgumentParser(description="Vectorized random-goal obstacle waypoint debug.")
...
def random_goal_waypoint_policy(env, obs_tensor, state):
    ...
    # phase=0 鐨勬椂鍊欑‘瀹氱粫琛屾柟鍚戯細
    # Choose the passing side from the target y: upper lane for y >= 0, lower lane otherwise.
    ...
    # phase 0 -> phase 1:
    # Switch after the robot has reached the lane and is near the obstacle x.
    ...
    # phase 1 -> phase 2:
    # Switch to the final goal after passing the obstacle.
...
def main():
    ...
    print("[INFO] random goal range:", cfg.goal_x_min, cfg.goal_x_max, cfg.goal_y_min, cfg.goal_y_max)
    ...
    state = RandomWaypointState(num_envs=args_cli.num_envs, device=env.device)
    ...
    print("[INFO] random vec obstacle debug reset done")
    print("[INFO] obs shape:", obs_tensor.shape)

    for step in range(args_cli.steps):
        action, target = random_goal_waypoint_policy(env, obs_tensor, state)
        ...
        if step % args_cli.print_every == 0:
            ...
            print(
                ...
                f"env0_side={state.pass_side[0].item():+.0f} | "
