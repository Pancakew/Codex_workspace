...
parser = argparse.ArgumentParser(description="Behavior cloning for FW-mini obstacle navigation.")
...
parser.add_argument("--save-dir", type=str, default="logs/fwmini_obstacle_bc")
...
def waypoint_expert_policy(env, obs_tensor, state):
...
def main():
    ...
    state = ObstacleWaypointState(num_envs=args_cli.num_envs, device=device)

    print("[INFO] Start collecting obstacle expert data")
    ...
    for step in range(args_cli.collect_steps):
        with torch.no_grad():
            expert_action = waypoint_expert_policy(env, obs_tensor, state)
        ...
        if torch.any(valid_mask):
            obs_chunks.append(obs_tensor[valid_mask].detach().clone())
    ...
    print("[INFO] Start obstacle BC training")
    ...
    checkpoint = {
        ...
        "task": "fwmini_obstacle_bc",
    }

    latest_path = os.path.join(args_cli.save_dir, "latest.pt")
    policy_path = os.path.join(args_cli.save_dir, "obstacle_bc_policy.pt")
